library(corncob)
context("Tests that use SummarizedExperiment")
...
if (requireNamespace("SummarizedExperiment", quietly = TRUE)) {
  soil_phylo_ps <- phyloseq::phyloseq(
    phyloseq::sample_data(soil_phylo_sample),
    phyloseq::otu_table(soil_phylo_otu, taxa_are_rows = TRUE),
    phyloseq::tax_table(soil_phylo_taxa)
  )

  # make SummarizedExperiment objects from data
  soil_phylo <- SummarizedExperiment::SummarizedExperiment(
    colData = data.frame(phyloseq::sample_data(soil_phylo_sample)),
    assays = list(counts = data.frame(phyloseq::otu_table(soil_phylo_otu, taxa_are_rows = TRUE))),
    rowData = data.frame(phyloseq::tax_table(soil_phylo_taxa))
  )
  soil_phylum_small <- SummarizedExperiment::SummarizedExperiment(
    colData = data.frame(phyloseq::sample_data(soil_phylum_small_sample)),
    assays = list(counts = data.frame(phyloseq::otu_table(soil_phylum_small_otu, taxa_are_rows = TRUE)))
  )
  soil_phylum_small_contrasts <- SummarizedExperiment::SummarizedExperiment(
    colData = data.frame(phyloseq::sample_data(soil_phylum_contrasts_sample)),
    assays = list(counts = data.frame(phyloseq::otu_table(soil_phylum_contrasts_otu, taxa_are_rows = TRUE)))
  )

  # tests for bbdml()
  test_that("bbdml works with SummarizedExperiment object", {
  ...
  set.seed(1)
  soil <- phyloseq::subset_samples(soil_phylo_ps, DayAmdmt %in% c(11,21))
  subsoil <- phyloseq::prune_taxa(x = soil, taxa = rownames(phyloseq::otu_table(soil))[301:325])
  subsoil_ps <- phyloseq::prune_taxa(x = soil, taxa = rownames(phyloseq::otu_table(soil))[301:325])
  ...
  subsoil <- SummarizedExperiment::SummarizedExperiment(
    colData = data.frame(phyloseq::sample_data(subsoil)),
    assays = list(counts = data.frame(phyloseq::otu_table(subsoil, taxa_are_rows = TRUE))),
    rowData = data.frame(phyloseq::tax_table(subsoil))
  )

  subsoil_disc <- SummarizedExperiment::SummarizedExperiment(
    colData = data.frame(phyloseq::sample_data(subsoil_disc)),
    assays = list(counts = data.frame(phyloseq::otu_table(subsoil_disc, taxa_are_rows = TRUE))),
    rowData = data.frame(phyloseq::tax_table(subsoil_disc))
  )

  subsoil_nonint <- SummarizedExperiment::SummarizedExperiment(
    colData = data.frame(phyloseq::sample_data(subsoil_nonint)),
    assays = list(counts = data.frame(phyloseq::otu_table(subsoil_nonint, taxa_are_rows = TRUE))),
    rowData = data.frame(phyloseq::tax_table(subsoil_nonint))
  )
  ...
  temp_sing_ps <- differentialTest(formula = ~ DayAmdmt,
                                phi.formula = ~ DayAmdmt,
                                formula_null = ~ 1,
                                phi.formula_null = ~ 1,
                                data = subsoil_ps, boot = FALSE, test = "LRT",
                                inits = rbind(rep(.01, 4)))
  ...
  mydat <- SummarizedExperiment::assay(subsoil)
  mysampdat <- SummarizedExperiment::colData(subsoil)
  ...
  test_that("differentialTest works without SummarizedExperiment", {
    expect_true(all.equal(temp_sing$p, temp_nonphylo$p))
  })

  test_that("differentialTest phyloseq and SummarizedExperiment are identical", {
    expect_true(all.equal(temp_sing$p, temp_sing_ps$p))
  })


  test_that("requires data frame, matrix, phyloseq, or SummarizedExperiment", {
