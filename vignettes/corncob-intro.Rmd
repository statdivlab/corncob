...
First, let's plot our model on the relative abundance scale. To do this, we simply type:

```{r}
plot(corncob)
```

The points represent the relative abundances. The bars represent the 95\% confidence intervals for the relative abundance by sample. 

Now let's look at the same model, but on the absolute abundance scale with 95\% prediction intervals (since absolute abundance is not a parameter). To do this, we add the option \texttt{AA = TRUE} to our plotting code.
...
In this case, we identified 4 taxa that are differentially-variable across \texttt{DayAmdmt} (out of the 40 taxa tested).
...
While the model fits, we should be skeptical of **any** statisical model fit on a single observed count, including \texttt{corncob}!

\texttt{corncob} is stable, but new. If you notice and issues, please [log them on github](https://github.com/bryandmartin/CORNCOB/issues) to help us help you!
