...
\item \texttt{Amdmt}: Categorical variable representing one of three soil additives; none, biochar, and freshbiomass, respectively.
...
The parameter `B` determines the number of bootstrap simulations used to approximate the prediction intervals. For purposes of this tutorial, we use a small value `B = 100` for computational purposes.

Now let's look at the same plot, but on the counts scale with 95\% prediction intervals (since counts is not a parameter). To do this, we add the option \texttt{total = TRUE} to our plotting code.

```{r}
plot(corncob, total = TRUE, B = 100)
```
...
```{r}
plot(corncob, total = TRUE, color = "DayAmdmt", B = 100)
```

```{r}
plot(corncob, color = "DayAmdmt", B = 100)
```
...
Let's try modeling the expected relative abundance and the variability of the counts with \texttt{DayAmdmt} as a covariate. We do this by modifying \texttt{formula} and \texttt{phi.formula} as:
...
```{r}
plot(corncob_da, color = "DayAmdmt", total = TRUE, B = 100)
```

```{r}
plot(corncob_da, color = "DayAmdmt", B = 100)
```
...
The difference between the formulas and the null version of the formulas are the variables that we test. We will go through several examples, starting with a test for differential abundance across the \texttt{DayAmdmt} coefficient.
...
In this case, we identified `r length(da_analysis$significant_taxa)` taxa that are differentially-abundant across \texttt{DayAmdmt} (out of the 39 taxa tested).
...
We will now walk through several scientific questions of interest and show how they can be answered using hypothesis testing with \texttt{corncob}. Note that Day and Amdmt are both factor covariates with levels 0, 1, and 2.
...
We can make the plot cleaner using the \texttt{level} parameter. Here we will display both the family and genus information.
