...
We can switch the OTU labels to taxonomic labels using \texttt{otu_to_taxonomy}. We supply our OTU labels as strings for the \texttt{OTU} argument. We supply the \texttt{phyloseq} object for the \texttt{data} argument.

```{r}
otu_to_taxonomy(fullAnalysis$DA, soil)
```

```{r}
otu_to_taxonomy(fullAnalysis$DV, soil)
```
...
While the model fits, we should be skeptical of **any** statisical model fit on a single observed count!
